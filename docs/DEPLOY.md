...
- 개인 키도 `설치.bat` 이 처리한다 — `~/.ssh/<access.key_name>` 을 `SECSH\UserKeys\` 에 넣거나(교재와 콘솔이 그 이름으로 만들게 한다), 이미 등록된 키가 하나뿐이면 세션의 `UserKey=` 를 그 이름에 맞춘다. 둘 다 아니면 화면에 절차를 띄운다.
- Xshell 은 `[가져오기]` 할 때 키를 자기 형식(`NSSSH`)으로 바꿔 저장한다. 넣어 둔 OpenSSH 키를 그대로 읽지 못하는 버전이면 한 번은 가져오기가 필요하고, 그 안내도 스크립트가 띄운다.
