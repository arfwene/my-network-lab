...
## 2. `exams` 테이블이 없었던 이유 — **미해명, 증거 소실**

*발견: 2026-08-24, 재설치 직후 운영 서버*

콘솔이 `active (running)` 으로 멀쩡히 떠 있는데 `var/console.db` 에 `exams` 가 없었다.
마감 스위퍼가 20초마다 `no such table: exams` 를 찍어 로그를 덮었다.

**왜 미해명인가**
`exams` 는 최초 커밋부터 `SCHEMA` 안에 있고(`git log -S` 확인), `db.init()` 은 기동 때
무조건 돌고, `CREATE TABLE IF NOT EXISTS` 라 몇 번을 돌려도 안전하다.
새 DB 를 만들어 보면 정상적으로 생긴다. **코드로는 설명되지 않는다.**

다시 설치하니 증상이 사라졌고, 그 과정에서 문제의 DB 파일이 사라졌다.
원인을 지목할 증거가 없다 — 추측을 결론으로 적지 않는다.

**가장 유력했던 가설 (검증 못 함)**
콘솔을 돌리는 계정이 아닌 다른 계정(대개 `sudo`)이 `var/console.db` 를 **먼저**
만들어 빈 파일이 생겼고, 그 뒤 콘솔이 그 파일을 고치지 못한 채 떴다.

**이미 넣어 둔 것**
- `db.init()` 이 스키마를 확인한다. 빠진 테이블이 있으면 기동 때 죽는다 —
  반쪽 스키마로 조용히 뜨지 않는다.
- 스위퍼가 `no such table` 을 만나면 `db.init()` 을 한 번 불러 스스로 고친다.
  못 고치면 같은 오류는 20분에 한 번만 찍는다.
- `make doctor` / 콘솔 [설치] 화면이 **DB 소유자 · DB 스키마**를 본다.
  `sqlite3` 명령이 없는 서버에서도 확인된다.

**다시 나면 이것부터 받을 것** (지우기 전에)
```sh
ls -l var/console.db          # 소유자가 콘솔 계정이 아니면 가설 확정
make doctor                   # DB 소유자 · DB 스키마 줄을 본다
journalctl -u my-network-lab --no-pager | head -50   # 기동 직후 로그
```
