...
처음 넣은 것도 바꾼 것도 똑같이 건다
  한때는 **바뀌었을 때**만 손으로 [지금 랩에 반영] 을 누르게 했다. 실습 도중에
  랩 작업이 갑자기 뜨는 것이 싫어서였는데, 누르는 것을 잊은 사람이 첫 홉에서
  막히고 이유를 알지 못했다. 거는 것은 `keys` 태그뿐이라 설정을 건드리지
  않으므로, 실습 중에 돌아도 잃는 것이 없다.
"""
import asyncio
import sys
