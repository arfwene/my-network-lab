...
def _job_done(j):
    ...
    if _drill_passed(j):
        ...
        # 중간 점검은 **가려 놓고** 푸는 것이라 스스로 주입해 푸는 것보다 어렵다.
        # 그걸 통과했는데 "장애 실습이 남았습니다" 가 계속 뜨면 말이 안 된다.
        # 같은 것을 더 어렵게 해낸 것이므로 그 모듈의 장애 실습으로 인정한다.
        db.update_progress(j.user, j.module, drill_passed=True)
