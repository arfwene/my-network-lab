...
무엇을 **안** 하는가
  · Proxmox 호스트에 **적용되지 않은 네트워크 변경**이 남아 있으면 ② 를 건너뛴다.
    ...
    이건 콘솔이 대신 판단할 일이 아니다. 그때만 화면에 버튼이 남는다.
...
def status():
    ...
    return {
        ...
        "steps": [{"id": s, "label": LABEL[s], "done": _done.get(s),
                   "blocked": _blocked.get(s), "action": BUTTON[s]}
                  for s in STEPS],
        "blocked": {s: r for s, r in _blocked.items() if r},
    }


def _mark(step, ok, why=""):
    if ok:
...
async def _step_access():
    ...
    try:
        ...
        _mark("access", False, f"{type(e).__name__}: {e}")
...
async def _run(runner, step, action, why_busy="다른 설치 작업이 도는 중이다"):
    ...
    try:
        ...
        _mark(step, False, getattr(e, "message", None) or str(e))
        return False
    except (RuntimeError, ValueError) as e:                 # 잠김 · 잘못된 요청
        _mark(step, False, f"{why_busy} ({e})")
        return False
    ...
    if job.status == "ok":
        _mark(step, True)
        return True
    _mark(step, False, f"작업이 실패했다 (종료 코드 {job.rc})")
    ...
    try:
        ...
        if not pve.confirmed():
            for s in ("mgmt-bridge", "mgmt-net"):
                _blocked[s] = "Proxmox 연결이 아직 확인되지 않았다"
        else:
...
async def _step_mgmt(runner):
    try:
        ...
        _blocked["mgmt-bridge"] = f"Proxmox 를 읽지 못했다: {type(e).__name__}"
        return
...
async def _step_jump(runner):
    ...
    if ready_jump and not ready_jump():
        _blocked["jump"] = ("콘솔이 점프 계정을 직접 적용할 수 없다 — "
                            "./install.sh --no-apt 를 한 번 실행할 것")
        return
