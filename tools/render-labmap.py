...
def render(stage="m11", lab_id=None):
    ...
    A("")
    # 이 지도는 늘 최종 단계로 그린다. 그런데 랩은 모듈마다 조금씩 자란다.
    # 그 차이를 적어 두지 않으면, M0 에서 지도를 연 사람이 자기 랩에 없는
    # 라우터 넷을 보고 "내 랩이 잘못됐나" 로 읽는다.
    A("이 지도는 **최종 모습**입니다. 지금 랩에는 아직 없는 장비도 그려져 있고,")
    A("모듈을 지날 때마다 하나씩 등장합니다. 지금 내 노드에 실제로 무엇이")
    A("올라와 있는지는 노드에서 `lab-whereami` 로 확인해 주세요.")
    A("")
    A("---")
    A("")
    A("## 1. 전체 토폴로지")
